    ...
    <div class="mb-8 bg-white p-6 rounded-xl shadow-sm border border-slate-200 print:hidden">
      ...
      <div class="flex space-x-4 items-end">
        <div class="flex-1">
          ...
          <select v-model="selectedStudent" class="w-full border p-2 rounded-lg focus:ring-2 focus:ring-blue-900 outline-none">
        ...
        <div class="w-48">
          ...
          <select v-model="selectedTerm" class="w-full border p-2 rounded-lg focus:ring-2 focus:ring-blue-900 outline-none">
        ...
        <button @click="loadReport" :disabled="!selectedStudent" class="bg-blue-900 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-800 transition disabled:bg-gray-300">
          Generate Report
        </button>
      </div>
    </div>

    <div v-if="reportData" class="bg-white max-w-4xl mx-auto p-12 rounded-xl shadow-lg border border-gray-200 print:shadow-none print:border-none print:m-0 print:p-0">
      ...
      <div class="text-center border-b-4 border-blue-900 pb-6 mb-8">
        ...
        <p class="text-lg text-gray-700 font-medium mt-2">Competency-Based Curriculum (CBC) Performance Report</p>
      </div>

      <div class="flex justify-between items-center bg-blue-50 p-6 rounded-lg mb-8 border border-blue-100">
        <div>
        ...
        <div class="text-right">
          ...
          <p class="text-lg font-bold text-gray-900">{{ reportData.grade_level }} • {{ reportData.term }}</p>
      ...
      <table class="w-full mb-12 border-collapse border border-gray-300">
        <thead>
          <tr class="bg-gray-100">
            <th class="border border-gray-300 p-3 text-left font-bold text-gray-800 uppercase text-sm w-1/3">Learning Area</th>
            <th class="border border-gray-300 p-3 text-center font-bold text-gray-800 uppercase text-sm w-1/6">Score</th>
            <th class="border border-gray-300 p-3 text-left font-bold text-gray-800 uppercase text-sm">Teacher's Remarks</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="result in reportData.results" :key="result.learning_area">
            <td class="border border-gray-300 p-3 font-medium text-gray-900">{{ result.learning_area }}</td>
            <td class="border border-gray-300 p-3 text-center font-black text-lg"
                :class="{'text-green-600': result.score === 'EE', 'text-blue-600': result.score === 'ME', 'text-orange-500': result.score === 'AE', 'text-red-600': result.score === 'BE'}">
              {{ result.score }}
            </td>
            <td class="border border-gray-300 p-3 text-gray-700 italic">{{ result.remarks || '---' }}</td>
          </tr>
          <tr v-if="reportData.results.length === 0">
            <td colspan="3" class="p-8 text-center text-gray-500 font-medium">No assessments recorded for this term yet.</td>
          </tr>
        </tbody>
      </table>

      <div class="bg-gray-50 border border-gray-200 p-6 rounded-lg mb-12">
        <h4 class="font-bold text-gray-800 uppercase text-sm mb-4 border-b pb-2">CBC Grading Key</h4>
      ...
      <div class="mt-16 text-center print:hidden">
        <button @click="printDoc" class="bg-gray-900 text-white px-8 py-3 rounded-lg shadow-lg hover:bg-black font-bold">
          🖨️ Print Report Card
        </button>
      </div>
    </div>
